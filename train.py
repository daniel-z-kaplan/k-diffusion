...
def main():
    ...
    p.add_argument('--dinov2-model', type=str, default='vitl14',
     ...
     # TODO: running the script in inference-only mode could be useful too
    p.add_argument('--do-train', action=argparse.BooleanOptionalAction,
                   help='whether to enable training (pass --no-do-train if you only want evaluation)')
    ...
    # TODO: evaluate-at-end would be more useful for training-enabled cases, but I'm doing at-init for a smaller code diff
    p.add_argument('--evaluate-at-init', action='store_true',
                   help='whether to evaluate before entering train loop')
    ...
    if guided_diff is None:
        ...
        if args.do_train:
            model = wrap_diffusion_model(inner_model, guided_diff)
        model_ema = wrap_diffusion_model(inner_model_ema, guided_diff)
    ...
    evaluate_enabled = args.evaluate_every > 0 and args.evaluate_n > 0 or args.evaluate_at_init
    if evaluate_enabled:
